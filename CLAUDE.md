...
- **`local/session.lua`** — Local review session lifecycle, parallel in shape to `init.lua` start/stop/reload. `start(base)` resolves the base ref (arg → `current.json` → default branch), computes merge-base, synthesizes `changed_files` from `git diff --name-status/--numstat` + untracked files (pure helpers: `status_word`, `resolve_rename_path`, `parse_name_status`, `parse_numstat`, `build_changed_files`), creates or resumes the session (`.fude/current.json`), sets state (`review_mode = "local"`, `local_session`), reuses `init.setup_review_autocmds`, registers local-only autocmds (BufWritePost re-anchor, BufEnter tracker sync), and starts the auto-reload timer. `stop()` tears down and clears `current.json` (the JSONL file is kept). `reload(silent)` re-reads git state and the JSONL synchronously.
...
| `review_mode` | init, local/session | init, scope, comments, overview, files, ui/comment_browser, local/session, local/tracker |
| `local_session` | local/session | comments, comments/local_sync, files, local/session |
