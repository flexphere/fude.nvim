...
- **`comments.lua`** — Fetches PR review comments, builds a `comment_map[path][line]` lookup, and provides navigation (`next_comment`/`prev_comment` with wrap-around), creation (single-line and multi-line range as drafts), viewing, reply, draft management, and review submission (`submit_as_review()` for batched GitHub reviews).
- **`ui.lua`** — All floating window UI: comment input editor (markdown buffer with `<CR>` save draft / `q` cancel, or `submit_on_enter` mode for immediate submit), comment viewer, PR overview window, and review event selector (`select_review_event()`). Manages extmarks (virtual text) for comment indicators on lines.
