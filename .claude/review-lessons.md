...
### エッジケース: 外部コマンドの非ゼロ終了を一律「対象なし」として扱う (PR #191, 2026-09-25)
- **問題**: `gh pr view <branch>`はPRが無いときも認証切れ・ネットワーク障害のときも非ゼロで終了する。全エラーを「PRなし」に変換したため、ユーザーに「open PRがない」と誤った原因を伝えていた
- **対策**: 「見つからない」をエラー終了で表すCLIを呼ぶときは、not-foundのメッセージ（`no pull requests found`等）だけを「対象なし」として扱い、それ以外はエラー内容を呼び出し元へ返して別の通知にする。判定は`is_*_error`のような関数に切り出してテストする
- **該当箇所**: lua/fude/gh.lua, lua/fude/pr.lua

### 堅牢性: 同期パスで候補数に比例してプロセスを起動する (PR #191, 2026-09-25)
- **問題**: picker表示前の同期処理で、候補branchごとに`git rev-list --count`を`vim.system():wait()`で起動していた。候補が多いリポジトリではUIが目に見えて止まる
- **対策**: UI表示前に走る同期処理では、外部コマンドの起動回数が入力件数に比例しないようにする。gitなら`git log --decorate-refs`や`for-each-ref --format`のように1回で必要な情報（所属と順序）を取れる呼び方を先に探す
- **該当箇所**: lua/fude/diff.lua

### テスト: 実gitリポジトリを作るテストで作者情報をグローバル設定に依存する (PR #191, 2026-09-25)
- **問題**: 一時repoで`git commit`には`-c user.name=...`を付けたが、`git commit-tree`には付け忘れた。ローカルはグローバル設定で通り、作者情報の無いCIだけ`Author identity unknown`で落ちた
- **対策**: テストで一時repoを作ったら、直後に`git config user.name/user.email`をrepoローカルに設定する（コマンドごとの`-c`指定に頼らない）。ローカル確認は`HOME`を空ディレクトリに向けてグローバル設定を切った状態でも行う
- **該当箇所**: tests/fude/diff_spec.lua
