...
- **Lua パターン構文**: Lua パターンは正規表現ではない。主な違い: `|` 交替なし、エスケープは `\` ではなく `%`（例: `%.` `%(`）、文字クラスは `%d` `%a` `%w` 等。量指定子は `*`（0 回以上） `+`（1 回以上） `-`（0 回以上・最短一致） `?`（0 回か 1 回）の4つ。正規表現と違い量指定子を適用できるのは単一の文字クラスのみで、グループ `(...)` には適用できない。CRLF 行末は `text = text:gsub("\r\n", "\n")` で `\r\n` を `\n` に正規化し、必要に応じて `text = text:gsub("\r", "\n")` で単独 `\r` も処理する等の段階的な変換で扱う
...
- [ ] docstring / 型注釈（`@param`/`@return` 等）と実装シグネチャが一致しているか（引数追加・型変更・削除が LuaDoc にも反映されているか）
- [ ] 追加・変更・削除した識別子（コマンド名・config キー・公開関数名）を `doc/fude.txt`, `README.md`, `CLAUDE.md`, `CONTRIBUTING.md` の 4 ファイルに対し grep し、説明の追従・削除漏れがないか確認したか
- [ ] skill ドキュメント内の自己参照（Phase 番号、用語の件数定義 = 行数 vs エントリ数 等）が一貫しているか確認したか
- [ ] 「全 X で同じ挙動」と謳う UI 動作（キーマップ、確認ダイアログ、通知等）は、類似する**全 variant**（`open_comment_input` / `open_edit_window` / `open_reply_window` / `comment_browser` 等）を Grep で列挙して各実装に存在するか確認したか。並行する複数実装はドリフトしやすい
...
- [ ] `pairs()` でテーブルを走査して順序が意味を持つ配列を生成していないか（表示用データ・テスト出力は `vim.tbl_keys` + `table.sort` + `ipairs` を使う）
- [ ] 既に取得済みの値（`repo_root` 等）を helper 経由で再取得していないか（引数を取る低レベル関数 `diff.make_relative` 等を直接呼ぶ）

### 堅牢性

- [ ] 非同期処理で state やディスク上ファイル（`drafts.json` 等の永続化ファイル）を楽観的に更新/削除していないか（失敗時のロールバック有無）。「先に削除→ API 待ち」は危険、「API 成功後に削除」が正解
...
- [ ] autocmd callback で `vim.api.nvim_get_current_buf()` ではなく `ev.buf` で対象バッファを明示しているか（`vim.schedule` 越しでも誤バッファ適用を防ぐ）
...
- [ ] ユーザー定義関数（`config.format_path` 等）の戻り値を `pcall` + type チェックで保護し、元の値にフォールバックしているか
- [ ] 非同期 callback 内で picker / window / buffer の有効性確認または `pcall` 保護をしているか（コールバック発火時に閉じられている可能性）
- [ ] 編集機能で読み込んだデータが `vim.trim` 等で意図しない差分を生まないか（読み込み→保存の往復で同一になるか）
- [ ] 新規 `gh pr view` / `gh pr edit` 呼出で detached HEAD 対策を適用しているか（ブランチ推論を避け、PR 番号を事前解決して渡す）
- [ ] GraphQL / REST API 呼出で `first: N` / `--limit N` 等の上限値を指定する際、想定する最大件数（コメント数 100+ 等）を超えないか、超えた場合の取りこぼし対策があるか確認したか
- [ ] 設定オプションが「無効」を示すとき（`outdated.show=false`、`auto_reload.enabled=false` 等）、関連する重い処理（GraphQL 呼出、データ取得、タイマー起動等）もスキップされているか確認したか
- [ ] 自動復元したデータ（保存済み draft 等）を入力バッファへ prefill した場合、コンテキスト切替時に「ユーザー編集中テキスト」と「未編集の復元データ」を区別して、未編集の復元データだけを差し替えているか。あわせて dirty 判定の original（基準）も復元データに合わせ、未編集なのに dirty 判定される不整合を防いでいるか
- [ ] ウィンドウ（特に float）を閉じる前の callback（保存/破棄ハンドラ等）から、`vim.api.nvim_get_current_buf()` に依存する副作用（extmark 再描画等）を直接呼んでいないか。close 前に走るパスでは `vim.schedule()` で遅延させ、フォーカスが元バッファへ戻った後に走らせる
- [ ] 永続化ファイル（`drafts.json` 等）からロードした値は、ソース（getter/loader）で型を検証してから返しているか。JSON として妥当でもフィールド型が想定外（`body` が数値、混在型等）になり得るため、外部入力として扱い、不正な型は安全側（nil 返却 / スキップ）に倒す

### テスト

- [ ] 追加したアサーションが既存と式変形で同値になっていないか（別観点を意図するならテスト名・コメントで表現し、テスト自体は重複させない）
- [ ] 「呼ばれないこと」を検証するテストで固定 sleep を使っていないか（`vim.wait(timeout, function() return invoked end)` で condition 不成立を検出する）
- [ ] テストの副作用で `vim.cmd("edit ...")` 等の架空バッファを作っていないか（`helpers.mock(vim, "cmd", capture_fn)` でコマンド文字列をキャプチャする）
- [ ] `vim.schedule` 経由のモックコールバックを `helpers.wait_for` で待つテストで、CI 負荷下でも安定するタイムアウトを使っているか（`helpers.wait_for` のデフォルト 5000ms をそのまま使う。短時間で失敗させたい「呼ばれない」検証は `vim.wait` を直接 50-100ms で呼ぶ）
