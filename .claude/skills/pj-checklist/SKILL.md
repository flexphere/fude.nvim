...
- [ ] 複数の並列関数・variantに同じ変更を加えたとき、検証を代表1件だけに書いていないか（変更対象の数とアサーション対象の数を数え直してから「全て検証」と書く）
- [ ] 設定値から動作までを結ぶ機能（キーマップ、config → 分岐等）で、結線そのものを通るテストが1本あるか（登録されたcallbackを取り出して呼ぶ等。「この行を消したらどのテストが落ちるか」を自問する）
