...
- **idle_limit**: 無音が続いたら Ready 化するまでの秒数。既定 `900`（15分）。正の整数のみ
- **poll**: ポーリング間隔（秒）。既定 `60`。正の整数のみ。レート制限回避のため最小 `30`（下回る値は
  `30` に切り上げ）
...
- `REVIEW <id> @<author> <state>` — 新規 PR レビュー（Copilot サマリ、APPROVE/CHANGES_REQUESTED 等）。
  **body（レビュー本文・指摘内容）は流れず `state` のみ**を通知する。詳細は `/develop-review-respond`
  側で API から取得して判断する
- `ISSUE <id> @<author> <url>` — 新規会話コメント
- `POLL_ERROR <n> ...` — `gh api` が `n` 回連続失敗（通信断・レート制限等）。待受は継続するが、
  API 失敗は「新規活動なし」とはみなさず idle タイマーを進めない（誤 Ready 化を防ぐ）。
  継続する場合はユーザーに状況を報告する
...
- **`COMMENT` / `REVIEW`（`CHANGES_REQUESTED` 等）**（`REVIEW` は `state` のみ通知されるため、
  指摘内容は `/develop-review-respond` 側で取得して判断する）:
